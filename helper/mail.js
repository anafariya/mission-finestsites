...
async function createEmail({ template, content, values }) {
  ...
  if (content) {
    ...
    content.preheader = content.preheader || content.body.split('\n')[0];

    // inject domain into button URL if needed
    ...
    if (values?.name && content.name !== 'contact') {
      content.body = `Hi ${escape(values.name)},\n\n${content.body}`;
    }

    // handle multiple \n and convert them into spacing
    content.body = content.body.split(/(\n+)/).map((line) => {
		if (/^\n+$/.test(line)) {
			const count = line.length; // count of \n
			return `<div style="height: ${count * 5}px;"></div>`;
		}

		// regular line
		return `<p style="font-family: 'Source Sans Pro', helvetica, sans-serif; font-size: 15px; font-weight: normal; Margin: 0; Margin-bottom: 15px; line-height: 1.6;">${line}</p>`;
    }).join('\n');
    ...
    email = email.replace('{{button.url}}', content.button_url || content.button?.url);
    email = email.replace('{{button.label}}', content.button_label || content.button?.label);
    email = email.replace('{{closing}}', content.closing);
